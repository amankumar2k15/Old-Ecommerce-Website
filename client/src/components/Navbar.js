                ...
                <div className={`${hide ? "hidden" : "visible"} w-full md:block md:w-auto`} id="navbar-solid-bg">
                    <ul className="flex flex-col font-medium mt-4 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-transparent dark:bg-gray-800 md:dark:bg-transparent dark:border-gray-700 gap-2 md:gap-0">
                        <li onClick={() => setHide(!hide)}>
                            <Link to="/home" className="block py-2 pl-3 pr-4 text-white bg-blue-700 rounded md:bg-transparent md:p-0 dark:text-white md:hover:text-blue-700 md:dark:bg-transparent" aria-current="page">Home</Link>
                        </li>
                        <li onClick={() => setHide(!hide)}>
                            <Link to="/product" className="block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">Product</Link>
                        </li>
                        <div className='relative 
                         h-8 w-10 ml-2'>
                            <Link to="/cart"
                                onClick={() => setHide(!hide)}>
                                <AiOutlineShoppingCart className='text-white text-2xl cursor-pointer md:hover:text-blue-700' />
                            </Link>
                            <span className='absolute top-0 right-0 text-white rounded-full  bg-[red] w-5 text-[12px] text-center'>{items.length}</span>
                        </div>
                    </ul>
                </div>
            </div>
